//
//  XPBSixHeStatColorTableViewCell.m
//  XPSixHeLottery
//
...
@implementation XPBSixHeStatColorTableViewCell
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        CGFloat margant = (SCREENWIDTH/2 - 3 * 35 - 10)/2;
        for(int i = 0;i < 3; i++)
        {
            UIButton *btn = [UIButton new];
            [self addSubview:btn];
            [btn setTitle:@"红波" forState:UIControlStateNormal];
            btn.titleLabel.font = [UIFont systemFontOfSize:14];
            [btn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
            [btn addTarget:self action:@selector(didClickDataBtn:) forControlEvents:UIControlEventTouchUpInside];
            btn.frame = CGRectMake(SCREENWIDTH/2 + 5+ i * (20+ margant) , CGRectGetMinY(appearSNumLabel.frame), 35, 20);
            btn.tag = i;
        }
