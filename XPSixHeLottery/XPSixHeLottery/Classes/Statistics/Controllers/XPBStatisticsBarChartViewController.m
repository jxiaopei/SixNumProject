//
//  XPBStatisticsBarChartViewController.m
//  XPSixHeLottery
//
...
@interface XPBStatisticsBarChartViewController ()<UIPickerViewDataSource,UIPickerViewDelegate>
...
@implementation XPBStatisticsBarChartViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    _tipArr = @[@[@"特码生肖历史热图(出现的次数)",@"特码生肖历史冷图(遗漏的次数)"],
                @[@"正码生肖历史热图(出现的次数)",@"正码生肖历史冷图(遗漏的次数)"],
                @[@"特码波色历史热图(出现的次数)",@"特码波色历史冷图(遗漏的次数)"],
                @[@"正码波色历史热图(出现的次数)",@"正码波色历史冷图(遗漏的次数)"],
                @[@"特码尾数历史热图(出现的次数)",@"特码尾数历史冷图(遗漏的次数)"],
                @[@"正码尾数历史热图(出现的次数)",@"正码尾数历史冷图(遗漏的次数)"],
                ];
    _urlArr = @[StatisZodSpe,StatisZodNor,StatisColSpe,StatisColNor,StatisSpeMant,StatisNorMant];
...
-(void)getDataWithPeriods:(NSString *)period{
    ...
    if(_statisticsType == StatisSpeZodiacType || _statisticsType == StatisNorZodiacType){
        paramData = @{@"periods":[NSNumber numberWithInteger:period.integerValue],
                      @"prop_mode":@"生肖",};
    }else if (_statisticsType == StatisSpeColorType || _statisticsType == StatisNorColorType){
        paramData = @{@"periods":[NSNumber numberWithInteger:period.integerValue],
                      @"prop_mode":@"波色",};
    }else{
        paramData = @{@"periods":[NSNumber numberWithInteger:period.integerValue]};
        ...
        if([responseObject[@"code"] isEqualToString:@"0000"])
        {
            ...
            if(hotArr.count > 0){
                for(XPBSixHeStatDataModel *dataModel in hotArr){
                    ...
                    [self.hotTitleArr addObject:dataModel.prop_name];
                    
                    [self.hotDataSource addObject:[NSString stringWithFormat:@"%zd",dataModel.prop_count]];
                }
                if(_hotBarChart){
            ...
            if(coldArr.count > 0){
                for(XPBSixHeStatDataModel *dataModel in coldArr){
                    ...
                    [self.coldTitleArr addObject:dataModel.prop_name];
                    
                    [self.coldDataSource addObject:[NSString stringWithFormat:@"%zd",dataModel.prop_count]];
                }
...
-(void)setupHotBarChart{
    ...
    scrollView.contentSize = CGSizeMake(SCREENWIDTH, (SCREENHEIGHT - 124)/2);
...
-(PNBarChart *)hotBarChart{
    if(_hotBarChart == nil)
    {
        PNBarChart *barChart = [[PNBarChart alloc] initWithFrame:CGRectMake(0, 0, SCREENWIDTH, (SCREENHEIGHT - 124)/2)];
